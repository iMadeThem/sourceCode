 ...
 * It can be leveraged to support multi-level undo of the Command pattern. In this example,
 * before a command is run against the Number object, Number’s current state is saved in
 * Command’s static memento history list, and the command itself is saved in the static
 * command history list.  Undo() simply “pops” the memento history list and reinstates
 * Number’s state from the memento. Redo() “pops” the command history list.
...
int main()
{
    // can not deal with characters other than numbers, like a, b etc.
    int i;
    cout << "Integer: ";
    cin >> i;
    ...
    while (i){
        if(i < '0' || i > '9' || i > 4) {
            cout << "Bad command! Command should be 0~4 only!" << endl;
            cout << "Exit[0], Double[1], Half[2], Undo[3], Redo[4]: ";
            cin >> i;
            continue;
        }
