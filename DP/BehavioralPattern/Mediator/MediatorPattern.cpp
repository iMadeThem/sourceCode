 ...
 * proliferating interconnections tend to reduce it again. You can avoid this problem
 * by capsulating the interconnections (i.e. the collective behavior) in a separate
 * “mediator” object. A mediator is responsible for controlling and coordinating the interactions of a group of objects.
 *
 ...
 * Whenever a simulated user interaction occurs in a child widget Widget::changed(),
 * the widget does nothing except “delegate” that event to its parent dialog box mediator->widgetChanged(this).
 *
 * FileSelectionDialog::widgetChanged() encapsulates all collective behavior for the dialog box
 * (it serves as the hub of communication). The user may choose to “interact” with a simulated:filter
